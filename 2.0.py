...
def on_choose_file_or_folder():
    ...
    start_extract(paths, extract_to)
...
def on_choose_file_or_folder_for_compress():
    """选择要压缩的文件或文件夹"""
    # 先尝试选择文件
    file_path = filedialog.askopenfilename(title="选择要压缩的文件")
    if file_path:
        compress_target = file_path
        is_file = True
    else:
        # 如果未选择文件，尝试选择文件夹
        ...
        if not folder_path:
            return  # 用户取消了选择
        compress_target = folder_path
        is_file = False
    
    # 选择压缩包保存路径和格式
    save_compressed_file(compress_target, is_file)
...
def save_compressed_file(target_path, is_file):
    ...
    if is_file:
        default_name = os.path.splitext(default_name)[0]  # 去除原文件扩展名
...
if __name__ == "__main__":
    ...
    root.geometry("680x420")  # 稍微增加宽度以容纳所有按钮
    ...
    btn_extract = tk.Button(
        btn_frame, text="解压文件/文件夹", command=on_choose_file_or_folder,
        bg="#ff6b6b", fg="white", font=("微软雅黑", 11, "bold"),
        activebackground="#ff8787", activeforeground="white",
        relief="flat", padx=8, pady=4, bd=0, width=16
    )
    btn_extract.pack(side=tk.LEFT, padx=12, ipadx=8, ipady=2)

    btn_compress = tk.Button(
        btn_frame, text="压缩文件/文件夹", command=on_choose_file_or_folder_for_compress,
        bg="#4e9cff", fg="white", font=("微软雅黑", 11, "bold"),
        activebackground="#6eaaff", activeforeground="white",
        relief="flat", padx=8, pady=4, bd=0, width=16
    )
    btn_compress.pack(side=tk.LEFT, padx=12, ipadx=8, ipady=2)
