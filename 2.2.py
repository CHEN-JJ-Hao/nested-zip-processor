...
class Extractor:
    def __init__(self):
        ...
        self.extracted_dirs = []
        self.compressed_files = []  # 新增压缩文件记录
    ...
    def rollback(self):
        ...
        for d in reversed(self.extracted_dirs):
            if os.path.exists(d):
                try:
                    shutil.rmtree(d, ignore_errors=False)  # 不忽略错误，确保删除成功
    ...
    def compress_folder(self, folder_path, archive_path, fmt="zip"):
        self.compressed_files.append(archive_path)  # 记录压缩文件路径
...
def start_extract_file(file_paths, extract_to):
    ...
    extractor.compressed_files.clear()  # 清空压缩文件记录
...
if __name__ == "__main__":
    ...
    root.configure(bg="#f0f0f0")  # 浅灰色背景，更接近Windows风格
    ...
    sep1 = tk.Frame(root, bg="#d0d0d0", height=1)  # 细分隔线
    sep1.pack(fill=tk.X, padx=30, pady=(0, 15))

    drag_frame = tk.Frame(root, bg="#e6f2ff", bd=2, relief="ridge", height=100, width=400)  # 更明显的边框
    ...
    drag_label = tk.Label(
        ...
        font=("微软雅黑", 12, "bold"),  # 字体稍小
        bg="#e6f2ff",
        fg="#2c5282",
        anchor="center",
        justify="center",
        wraplength=380  # 限制文本宽度，自动换行
    )
    ...
    btn_frame.pack(side=tk.BOTTOM, pady=10)  # 底部按钮区域

    # 统一按钮样式参数
    btn_width = 14  # 增加宽度确保所有按钮都能完整显示文本
    btn_padx = 8
    btn_pady = 8
    btn_relief = "raised"  # 使用raised更符合Windows风格
    btn_bd = 1
    btn_active_bg = "#e0e0e0"  # 统一的活动背景色

    # 解压菜单按钮 - 优化样式
    extract_menu_btn = tk.Menubutton(
        btn_frame, text="解压",
        bg="#4a86e8", fg="white", font=button_font,  # 更亮的蓝色
    ...
    extract_menu_btn.pack(side=tk.LEFT, padx=5)  # 统一间距
    ...
    compress_menu_btn = tk.Menubutton(
        btn_frame, text="压缩",
        bg="#6aa84f", fg="white", font=button_font,  # 绿色按钮，区分功能
        activebackground=btn_active_bg, activeforeground="#333",
        relief=btn_relief, padx=btn_padx, pady=btn_pady, bd=btn_bd, width=btn_width
    )
    ...
    btn_pause_resume = tk.Button(
        ...
        bg="#f1c232", fg="#333", font=button_font,  # 黄色按钮
        activebackground=btn_active_bg, activeforeground="#333",
        relief=btn_relief, padx=btn_padx, pady=btn_pady, bd=btn_bd, width=btn_width
    )
    ...
    btn_stop = tk.Button(
        ...
        bg="#e66465", fg="white", font=button_font,  # 红色按钮
        activebackground=btn_active_bg, activeforeground="#333",
        relief=btn_relief, padx=btn_padx, pady=btn_pady, bd=btn_bd, width=btn_width
    )
    ...
    root.mainloop()
    extractor.rollback()  # 确保退出时清理所有解压内容
